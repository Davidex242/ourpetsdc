...
public class tag_desp extends RecyclerView.Adapter<tag_desp.Adapter_Desp> {
    private int is =1;
    private Context context;
    ArrayList<Info_Vacina_Desp> tag_desp;
    public tag_desp(Context ctx , ArrayList<Info_Vacina_Desp> g) {
        context = ctx;
        tag_desp = g;
    }
    @NonNull
    @Override
    public Adapter_Desp onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        return new tag_desp.Adapter_Desp(LayoutInflater.from(context).inflate(R.layout.list_vacinas, parent,false));
    }

    @Override
    public void onBindViewHolder(@NonNull final Adapter_Desp holder, final int i) {
        if (tag_desp.get(i).getTipo().equals("deworming") || tag_desp.get(i).getTipo().equals("desparazitação") || tag_desp.get(i).getTipo().equals("降级") || tag_desp.get(i).getTipo().equals("désescalade")) {
            holder.txtTipo.setText(tag_desp.get(i).getTipo());
            holder.txtDataVac.setText(tag_desp.get(i).getData());
            holder.txtHoraVac.setText(tag_desp.get(i).getHora());
            holder.txtNomeVac.setText(tag_desp.get(i).getNome());
            ...
            if (tag_desp.get(i).getStatus().equals("-")){
                holder.chkBoxVacina.setChecked(false);
            }else {
                holder.chkBoxVacina.setChecked(true);
            }
            final DatabaseReference RefStatus =   FirebaseDatabase.getInstance().getReference().child("boletim").child(FirebaseAuth.getInstance().getCurrentUser().getUid()).child(PetID).child(tag_desp.get(i).getKey());
                ...
                public void onClick(View v) {
                    ...
                    if (inDia >= 1 && inDia <= 31 && inMes >= 1 && inMes <= 12 && inAno >= 2020 &&  inHH >= 1 && inHH <= 24 && inMM >= 1 && inMM <= 60 ){
                        ...
                        FirebaseDatabase.getInstance().getReference().child("boletim").child(FirebaseAuth.getInstance().getCurrentUser().getUid())
                                .child(PetID).child(tag_desp.get(i).getKey()).updateChildren(updateMap).addOnCompleteListener(new OnCompleteListener() {
                            @Override
    ...
    public int getItemCount() {
        return tag_desp.size();
    }

    public class Adapter_Desp extends RecyclerView.ViewHolder {
        ...
        public Adapter_Desp(@NonNull View itemView) {
            super(itemView);
